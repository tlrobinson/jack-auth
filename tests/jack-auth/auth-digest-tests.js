 ...
 *
 * Acknowledgements:
 * Port of /test/spec_rack_auth_digest.rb from Rack project
...
var digestApp = DigestMD5.DigestMD5Middleware(openApp, {
    realm: myRealm,
...
var digestAppWithHashedPasswords = DigestMD5.DigestMD5Middleware(openApp, {
    realm: myRealm,
    opaque: "this-should-be-secret",
    passwordsHashed: true,
    getPassword: function(username) {
        return {'Alice': md5(["Alice", myRealm, "correct-password"].join(":"))}[username];
    }
});
...
var doRequestWithDigestAuth = function(request, method, path, username, password, options) {
    ...
    params.response =  DigestMD5.digest(params, password);
    headers['HTTP_AUTHORIZATION'] = "Digest "+DigestParams.toStr(params);
